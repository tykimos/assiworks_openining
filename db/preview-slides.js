#!/usr/bin/env node
/**
 * Presentation Preview Diagnostic Tool
 *
 * Usage:
 *   node db/preview-slides.js [presentation-id]
 *   node db/preview-slides.js          # uses latest presentation
 *
 * Outputs: db/slides-preview.pdf  (all slides as PDF)
 *          db/slide-*.png         (individual slide screenshots)
 */

const { chromium } = require('/Users/tykimos/.nvm/versions/node/v24.13.1/lib/node_modules/playwright');
const fs = require('fs');
const path = require('path');

const SUPABASE_URL = 'https://gpntqfvzikcrsomumwaa.supabase.co';
const SUPABASE_ANON_KEY = 'sb_publishable_GICJdo5wM5WVlHbV3DHbmQ_Epwvp9k9';

// ---- Rendering functions (extracted from admin.js) ----
  ...
  for (const line of lines) {
    ...
    if (m) {
      ...
      if (directive === 'cols') {
        ...
        else if (directive === 'right') html = (parent === 'cols' || parent === 'cols-3') ? '<div class="slide-col-right">' : '<div class="slide-align-right">';
...
const slideClassMap = {
  '.large': 'slide-text-large', '.small': 'slide-text-small',
  '.muted': 'slide-text-muted', '.accent': 'slide-text-accent',
  '.orange': 'slide-text-orange', '.center': 'slide-text-center',
  '.gradient': 'slide-text-gradient', '.glow': 'slide-text-glow',
  '.xl': 'slide-text-xl', '.xxl': 'slide-text-xxl',
  '.bold': 'slide-text-bold', '.light': 'slide-text-light',
  '.spacer': 'slide-spacer', '.spacer-lg': 'slide-spacer-lg',
  '.spacer-xl': 'slide-spacer-xl', '.spacer-sm': 'slide-spacer-sm',
  '.fade-in': 'slide-frag', '.fade-up': 'slide-fade-up',
  '.scale-in': 'slide-scale-in',
  '.white': 'slide-text-white', '.blue': 'slide-text-blue', '.sub': 'slide-text-sub',
};
  ...
  for (const line of lines) {
    ...
    for (const cls of allRawClasses) {
      if (cls === '.spacer' || cls === '.spacer-lg' || cls === '.spacer-xl' || cls === '.spacer-sm') {
        isSpacer = true; cssClasses.push(classMap[cls]);
      } else if (classMap[cls]) { cssClasses.push(classMap[cls]);
      } else if (!cls.startsWith('.')) { cssClasses.push(cls); }
    }
...
const renderMarkdown = (text) => {
  ...
  // Wrap consecutive slide-img tags in a flex row
  html = html.replace(
...
function buildHTML(slides, cssContent) {
  const slideCards = slides.map((slideText, i) => {
...
/* Override for screenshot mode: show all slides vertically */
body { margin: 0 !important; padding: 0 !important; background: transparent !important; display: block !important; min-height: auto !important; }
.slides-container {
...
.slide-frag, .slide-fade-up, .slide-scale-in { opacity: 1 !important; transform: none !important; }
</style>
</head>
<body>
...
async function main() {
  ...
  let url = `${SUPABASE_URL}/rest/v1/presentations?select=*&order=updated_at.desc&limit=1`;
  if (presId) url = `${SUPABASE_URL}/rest/v1/presentations?id=eq.${presId}`;
  ...
  if (!data.length) { console.error('No presentation found'); process.exit(1); }

  const pres = data[0];
  console.log(`Rendering: "${pres.title}" (${pres.id})`);

  const slides = splitSlides(pres.content);
  ...
  // Build HTML
  const htmlContent = buildHTML(slides, cssContent);
  const htmlPath = path.join(outDir, 'slides-preview.html');
  fs.writeFileSync(htmlPath, htmlContent, 'utf-8');
  console.log(`HTML: ${htmlPath}`);

  // Use Playwright: render one slide at a time for clean screenshots
  const browser = await chromium.launch();
  const page = await browser.newPage({ viewport: { width: 960, height: 540 } });

  for (let i = 0; i < slides.length; i++) {
    const singleHTML = buildHTML([slides[i]], cssContent);
    const singlePath = path.join(outDir, `_tmp_slide.html`);
    fs.writeFileSync(singlePath, singleHTML, 'utf-8');
    await page.goto(`file://${singlePath}`);
    await page.waitForTimeout(200);
    const imgPath = path.join(outDir, `slide-${String(i + 1).padStart(2, '0')}.png`);
    await page.screenshot({ path: imgPath, clip: { x: 0, y: 0, width: 960, height: 540 } });
    console.log(`  Slide ${i + 1}: ${imgPath}`);
  }
  fs.unlinkSync(path.join(outDir, '_tmp_slide.html'));

  // PDF: all slides
  const pdfPath = path.join(outDir, 'slides-preview.pdf');
  const pdfSlides = [];
  for (let i = 0; i < slides.length; i++) {
    pdfSlides.push(fs.readFileSync(path.join(outDir, `slide-${String(i + 1).padStart(2, '0')}.png`)));
  }
  // Simple HTML-based PDF with all slide images
  const pdfHTML = `<!DOCTYPE html><html><head><style>
    @page { size: 960px 540px; margin: 0; }
    body { margin: 0; }
    img { width: 960px; height: 540px; display: block; page-break-after: always; }
  </style></head><body>${pdfSlides.map((_, i) =>
    `<img src="file://${path.join(outDir, `slide-${String(i + 1).padStart(2, '0')}.png`)}" />`
  ).join('')}</body></html>`;
  const pdfHTMLPath = path.join(outDir, '_tmp_pdf.html');
  fs.writeFileSync(pdfHTMLPath, pdfHTML, 'utf-8');
  const pdfPage = await browser.newPage();
  await pdfPage.goto(`file://${pdfHTMLPath}`);
  await pdfPage.waitForTimeout(300);
  await pdfPage.pdf({ path: pdfPath, width: '960px', height: '540px', printBackground: true, margin: { top: 0, bottom: 0, left: 0, right: 0 } });
